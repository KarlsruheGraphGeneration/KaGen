...
namespace kagen {
    ...
    void Generator::GenerateEdgeWeights(EdgeWeightConfig weight_config, MPI_Comm comm) {
        std::unique_ptr<kagen::EdgeWeightGeneratorFactory> factory = CreateEdgeWeightGeneratorFactory(
                weight_config.edge_weight_type);
        std::unique_ptr<kagen::EdgeWeightGenerator> generator = factory->Create(weight_config);
        if (xadj_.empty()) {
        ...
        if (weight_config.edge_weight_type == EdgeWeightType::RANDOM) {
    ...
    void Generator::ValidateEdgeWeight(MPI_Comm comm) {
        ...
        std::vector<VertexRange> ranges = AllgatherVertexRange(vertex_range_, comm);
        const auto &[local_from, local_to] = ranges[rank];

        if (representation_ == GraphRepresentation::CSR) {
            edges_ = BuildEdgeListFromCSR(vertex_range_, xadj_, adjncy_);
        }
        // Exchanging edgelist
        Edgelist correspondingEdgeList;
        EdgeWeights correspondingEdgeWeights;
        GatherCorrespondingEdgelist(edges_, edge_weights_, ranges, correspondingEdgeList,
                                    correspondingEdgeWeights, comm);

        // ToDo: Algorithms use different sortings for their edgelist

        int id;
        for (int i = 0; i < edges_.size(); i++) {
            SInt tail = edges_[i].first;
            SInt head = edges_[i].second;

            if (tail < head) {
                if (head < local_from || head >= local_to) {
                    auto target = std::make_pair(head, tail);
                    auto it = std::lower_bound(correspondingEdgeList.begin(), correspondingEdgeList.end(), target);

                    if (it != correspondingEdgeList.end() && *it == target) {
                        id = std::distance(correspondingEdgeList.begin(), it);
                        edge_weights_[i] = correspondingEdgeWeights[id];
                    }

                } else {
                    auto target = std::make_pair(head, tail);
                    auto it = std::lower_bound(edges_.begin(), edges_.end(), target);

                    if (it != edges_.end() && *it == target) {
                        id = std::distance(edges_.begin(), it);
                        edge_weights_[i] = edge_weights_[id];
                    }
                }
            }
        }
...
//        if (rank == 0) {
//            for (int i = 0; i < recv_buf.size(); i += 2) {
//                std::cout << "Communicated Edge i " << i << " (" << recv_buf[i] << ", " << recv_buf[i + 1] << ") "
//                          << recv_buf_w[i / 2] << std::endl;
//            }
//        }
