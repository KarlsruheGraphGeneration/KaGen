...
PGeneratorConfig
Hyper_HyperbolicFactory::NormalizeParameters(PGeneratorConfig config, PEID, const PEID size, const bool output) const {
    if (config.k == 0) {
    ...
    if (config.avg_degree == 0) {
        ...
        config.avg_degree = 2.0 * config.m / config.n;
        ...
        config.n = static_cast<SInt>(2 * config.m / config.avg_degree);
    ...
    if (config.hp_floats == 0 && std::log2(config.n) > 29) {
        if (output) {
            std::cout << "Enabling high-precision FP for hyperbolic hypergraph generator" << std::endl;
        }
...
Hyper_Hyperbolic<Double>::Hyper_Hyperbolic(const PGeneratorConfig& config, const PEID rank, const PEID size)
    : CSROnlyGenerator(),
      config_(config),
      rank_(rank),
      size_(size),
...
void Hyper_Hyperbolic<Double>::BeginHyperedge(const Vertex& center) {
    ...
    current_hyperedge_radius_ = static_cast<Double>(getSampledOrConstantRadiusSquared(config_, center));
 ...
 */
template <typename Double>
void Hyper_Hyperbolic<Double>::EndHyperedge(const Vertex&) {
    ...
    for (SInt cell_id = 0; cell_id < GridSizeForAnnulus(annulus_id); ++cell_id) {
        ...
        for (SInt i = 0; i < vertices_[global_cell_id].size(); ++i) {
            ...
            if (pe_min_phi_ > std::get<0>(v) || pe_max_phi_ < std::get<0>(v))
                continue;
            BeginHyperedge(v);
            QueryHyperBallBoth(annulus_id, chunk_id, cell_id, v);
            EndHyperedge(v);
        }
    }
}
...
void Hyper_Hyperbolic<Double>::QueryHyperBallBoth(
    ...
    QueryHyperBall(annulus_id, chunk_id, cell_id, q);
    if (config_.query_both && annulus_id > 0) {
        ...
        QueryHyperBall(annulus_id - 1, chunk_id, next_cell_id, q, false);
    }
}
...
void Hyper_Hyperbolic<Double>::QueryHyperBall(
    ...
    if (search_down || !IsLocalChunk(chunk_id))
        GenerateGridPins(annulus_id, chunk_id, cell_id, q);
    ...
    */
    ...
    QueryHyperBall(next_annulus, chunk_id, next_cell_id, q, search_down);
}
...
void Hyper_Hyperbolic<Double>::GenerateGridPins(
    ...
    for (const Vertex& v: cell_vertices) {
        if (std::get<5>(v) == std::get<5>(q)) {
            continue;
        }

        if (PGGeometry<Double>::HyperbolicDistance(q, v) <= current_hyperedge_pdm_radius_) {
            current_hyperedge_pins_.push_back(std::get<5>(v));
        }
    }
}
