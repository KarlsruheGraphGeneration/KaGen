...
/**
 * Initializes hyperedge around center vertex
 *
 * @param center, the vertex the ball is formed around
 */
template <typename Double>
void Hyper_Hyperbolic<Double>::BeginHyperedge(const Vertex& center) {
    ...
    current_hyperedge_radius_ = static_cast<Double>(getSampledOrConstantRadius(config_, center));

    current_hyperedge_pdm_radius_ = (std::cosh(current_hyperedge_radius_) - 1.0) / 2.0;
}

/**
 * Finalizes collected pins and emits hyperedge
 */
template <typename Double>
...
void Hyper_Hyperbolic<Double>::GenerateHyperedges(const SInt annulus_id, const SInt chunk_id) {
    ...
    for (SInt cell_id = 0; cell_id < GridSizeForAnnulus(annulus_id); ++cell_id) {
        ...
        for (SInt i = 0; i < vertices_[global_cell_id].size(); ++i) {
            const Vertex center_vertex = vertices_[global_cell_id][i];
            if (pe_min_phi_ > std::get<0>(center_vertex) || pe_max_phi_ < std::get<0>(center_vertex)) {
                continue;
            }
            BeginHyperedge(center_vertex);
            QueryHyperBallBoth(annulus_id, chunk_id, cell_id, center_vertex);
            EndHyperedge(center_vertex);
        }
    }
}
...
void Hyper_Hyperbolic<Double>::QueryHyperBallBoth(
    const SInt annulus_id, const SInt chunk_id, const SInt cell_id, const Vertex& center) {
    ...
    if (config_.query_both && annulus_id > 0) {
        ...
        SInt   next_cell_id  = std::floor((std::get<0>(center) - min_chunk_phi) / grid_phi);
...
void Hyper_Hyperbolic<Double>::QueryHyperBall(
    const SInt annulus_id, const SInt chunk_id, const SInt cell_id, const Vertex& center, bool search_down) {
    /*if (std::get<5>(q) == 1280) {
    ...
    // Boundaries
    auto& annulus        = annuli_[ComputeGlobalChunkId(annulus_id, chunk_id)];
    auto  current_bounds = GetBoundaryPhis(std::get<0>(center), std::get<1>(center), annulus_id);
...
bool Hyper_Hyperbolic<Double>::QueryHyperBallRightNeighbor(
    const SInt annulus_id, SInt chunk_id, SInt cell_id, const Vertex& center, bool phase, bool search_down) {
    /*bool out = false;
    if (std::get<5>(q) == 1280) {
...
template <typename Double>
void Hyper_Hyperbolic<Double>::AddWholeCellPins(SInt global_cell_id, SInt center_id) {
...
void Hyper_Hyperbolic<Double>::GenerateGridPins(
    ...
    const auto relation = ClassifyCellAgainstHyperball(annulus_id, chunk_id, cell_id, q);
    ...
    if (relation == CellBallRelation::INSIDE) {
        AddWholeCellPins(global_cell_id, std::get<5>(q));
        return;
    }
    ...
    for (const Vertex& verticeToCheck: cell_vertices) {
        if (std::get<5>(verticeToCheck) == std::get<5>(q)) {
            continue;
        }

        if (PGGeometry<Double>::HyperbolicDistance(q, verticeToCheck) <= current_hyperedge_pdm_radius_) {
            current_hyperedge_pins_.push_back(std::get<5>(verticeToCheck));
        }
    }
}

/**
 * Distinguish cases where cell is contained in hyperball (no sampling of individual vertices)
 *                                   or partially contained in hyperball (sampling necessary)
 */
template <typename Double>
CellBallRelation Hyper_Hyperbolic<Double>::ClassifyCellAgainstHyperball(
    const SInt annulus_id, const SInt chunk_id, const SInt cell_id, const Vertex& hyperball_center) {
    ...
    const Double q_phi = std::get<0>(hyperball_center);
    const Double q_r   = std::get<1>(hyperball_center);

    // check if corners are inside hyperball. If all are contained, entire cell is contained.
    ...
    // Conservative: if not definitely inseide, treat as partial.
    //  TODO(clickup)[2026-05-15]: Suboptimal Classification of cell for Hyperball
    return CellBallRelation::PARTIAL;
}
/*
 * Check if point is contained in hyperball
 */
template <typename Double>
