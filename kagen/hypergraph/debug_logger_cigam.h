...
public:
    explicit CIGAMHypergraphDebugLogger(const std::string& filename, const bool append = false)
        : CsvDebugLogger(
              filename, append,
              "hyperedge_id,mode,hyperedge_size,dominant_vertex,dominant_final_vertex,layer,"
    ...
    void LogHyperedge(const CIGAMHyperedgeDebugEvent& event) {
        out() << event.hyperedge_id << ',' << event.mode << ',' << event.hyperedge_size << ',' << event.dominant_vertex
              << ',' << event.dominant_final_vertex << ',' << event.layer << ',' << event.endpoint_vertex << ','
              << event.block_j_min << ',' << event.block_j_max << ',' << event.block_log_population << ','
              << event.log_probability << ',' << event.sampling_attempts << ',' << event.duplicate_rejections << ','
              << event.duration_ns << '\n';
    }
};
