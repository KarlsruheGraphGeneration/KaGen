...
namespace kagen {
    ...
    void AddNonlocalReverseEdges(Edgelist &edge_list, const VertexRange vertex_range, MPI_Comm comm) {
        ...
        MPI_Alltoallv(
                send_buf.data(), send_counts.data(), send_displs.data(), MPI_UINT64_T, recv_buf.data(),
                recv_counts.data(),
    ...
    VertexRange RedistributeEdgesRoundRobin(Edgelist32 &source, Edgelist &destination, const SInt n, MPI_Comm comm) {
        ...
        MPI_Alltoallv(
                sendbuf.data(), send_counts.data(), send_displs.data(), MPI_LONG_LONG, recvbuf.data(),
                recv_counts.data(),
    ...
    void
    GatherCorrespondingEdgelist(Edgelist edges, EdgeWeights local_weights, std::vector<VertexRange> ranges,
        ...
        const auto &[local_from, local_to] = ranges[rank];
        std::unordered_map<PEID, std::vector<SInt>> buffer_edges;
        std::unordered_map<PEID, std::vector<SSInt>> buffer_weights;

        // Sorting the corresponding edges and weights for every pe
        for (int i = 0; i < edges.size(); i++) {
            SInt tail = edges[i].first;
            SInt head = edges[i].second;
            SSInt weight = local_weights[i];

            if (head < local_from || head >= local_to) {
                const SInt pe = static_cast<SInt>(FindPEInRange(head, ranges));

                buffer_edges[pe].emplace_back(tail);
                buffer_edges[pe].emplace_back(head);
                buffer_weights[pe].emplace_back(weight);
            }
        }
        ...
        std::vector<int> recv_displs(size);
        for (size_t i = 0; i < send_counts.size(); ++i) {
            send_counts[i] = buffer_edges[i].size();
        }
        ...
        send_buf.reserve(total_send_count);
        for (size_t i = 0; i < send_counts.size(); ++i) {
            for (const auto &elem: buffer_edges[i]) {
                send_buf.push_back(elem);
            }
            buffer_edges[i].clear();
            buffer_edges[i].resize(0);
        }
        recv_buf.resize(total_recv_count);
        MPI_Alltoallv(
                send_buf.data(), send_counts.data(), send_displs.data(), MPI_UINT64_T, recv_buf.data(),
        ...
        for (std::size_t i = 0; i < recv_buf.size(); i += 2) {
            correspondingEdges.emplace_back(recv_buf[i], recv_buf[i + 1]);
        }
        recv_buf.clear();
        recv_buf.resize(0);

        // Exchanging weights
        std::vector<SSInt> send_buf_w;

        for (size_t i = 0; i < send_counts.size(); ++i) {
            send_counts[i] = buffer_weights[i].size();
        }

        std::exclusive_scan(send_counts.begin(), send_counts.end(), send_displs.begin(), 0);
        const std::size_t total_send_count_w = send_displs.back() + send_counts.back();
        MPI_Alltoall(send_counts.data(), 1, MPI_INT, recv_counts.data(), 1, MPI_INT, comm);
        std::exclusive_scan(recv_counts.begin(), recv_counts.end(), recv_displs.begin(), 0);
        const std::size_t total_recv_count_w = recv_displs.back() + recv_counts.back();
        send_buf_w.reserve(total_send_count_w);
        for (size_t i = 0; i < send_counts.size(); ++i) {
            for (const auto &elem: buffer_weights[i]) {
                send_buf_w.push_back(elem);
            }
            buffer_weights[i].clear();
            buffer_weights[i].resize(0);
        }
        correspondingWeights.resize(total_recv_count_w);
        MPI_Alltoallv(
                send_buf_w.data(), send_counts.data(), send_displs.data(), MPI_UINT64_T, correspondingWeights.data(),
                recv_counts.data(),
                recv_displs.data(), MPI_UINT64_T, comm);
        send_buf_w.clear();
        send_buf_w.resize(0);
