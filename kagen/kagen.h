...
struct Graph {
    ...
    // Append another graph to this one. Both must use the same representation.
    // After the call, this graph covers the union of both vertex ranges (which must be contiguous).
...
public:
    ...
    /*!
     * Sets the number of threads used for intra-process parallel generation (Default: 1).
     * When threads > 1, each MPI process spawns @p threads worker threads, each acting as an
     * independent virtual PE. The results are merged into a single graph before returning.
     * This multiplies the effective PE count by @p threads.
     * Note: only supported for communication-free generators; generators that require
     * cross-PE communication during finalization (e.g. RHG with query_both=false) are
...
/*!
 * Returns an array A of size comm.Size() + 1 s.t. PE i contains nodes in the range [A[i], A[i + 1]).
 *
 * @param graph Graph generated by any of the Generate* functions.
 * @param idx_type CommDatatype corresponding to the template parameter IDX.
 * @param comm Communicator that was used to generate the graph.
 *
...
std::vector<IDX> BuildVertexDistribution(const Graph& graph, CommDatatype idx_type, Comm& comm) {
    ...
    comm.Allgather(COMM_IN_PLACE, 0, idx_type, distribution.data() + 1, 1, idx_type);
...
#ifndef KAGEN_NOMPI
/*!
 * Overload for backward compatibility: accepts a raw MPI communicator.
 * Maps MPI_Datatype to CommDatatype internally.
 */
inline CommDatatype MPIDatatypeToComm(MPI_Datatype mpi_type) {
    if (mpi_type == MPI_INT)                return CommDatatype::INT;
    if (mpi_type == MPI_UNSIGNED)           return CommDatatype::UNSIGNED;
    if (mpi_type == MPI_LONG_LONG)          return CommDatatype::LONG_LONG;
    if (mpi_type == MPI_UNSIGNED_LONG_LONG) return CommDatatype::UNSIGNED_LONG_LONG;
    if (mpi_type == MPI_DOUBLE)             return CommDatatype::DOUBLE;
    if (mpi_type == MPI_LONG_DOUBLE)        return CommDatatype::LONG_DOUBLE;
    if (mpi_type == MPI_UINT64_T)           return CommDatatype::UINT64;
    if (mpi_type == MPI_INT64_T)            return CommDatatype::INT64;
    if (mpi_type == MPI_C_BOOL)             return CommDatatype::C_BOOL;
    return CommDatatype::BYTE;
}

template <typename IDX>
std::vector<IDX> BuildVertexDistribution(const Graph& graph, MPI_Datatype idx_mpi_type, MPI_Comm comm) {
    MPIComm mcomm(comm);
    return BuildVertexDistribution<IDX>(graph, MPIDatatypeToComm(idx_mpi_type), mcomm);
}
#endif // KAGEN_NOMPI
