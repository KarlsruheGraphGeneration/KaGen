...
class GNMUndirected {
public:
    GNMUndirected(PGeneratorConfig& config, const PEID /* rank */, const PEID /* size */) : config_(config), rng_(config), io_(config) {}
    ...
    void Generate() {
        ...
        SInt row         = rank * num_chunks + ((SInt)rank >= leftover_chunks_ ? leftover_chunks_ : 0);
...
private:
    ...
    void QueryTriangular(
        ...
        if (row_id < offset_row + row_splitter) {
            ...
            QueryRowRectangle(
                ll_variate, num_rows / 2, column_splitter, row_id, offset_row + row_splitter, offset_column, level + 1);
            QueryTriangular(
                m - upper_variate - ll_variate, num_rows / 2, num_columns / 2, row_id, column_id,
                offset_row + row_splitter, offset_column + column_splitter, level + 1);
        }
    }
    ...
    void QueryColumnRectangle(
        ...
        if (column_id < offset_column + column_splitter) {
            ...
            QueryColumnRectangle(
                ll_variate, num_rows / 2, column_splitter, column_id, offset_row + row_splitter, offset_column,
                level + 1);
    ...
    void GenerateTriangularEdges(const SInt m, const SInt row_id, const SInt column_id) {
    ...
    inline HPFloat NumTriangleEdges(const HPFloat row, const HPFloat column, bool loops = false) const {
        return (loops && config_.self_loops) ? row * (column + 1) / 2 : row * (column - 1) / 2;
    }

    inline HPFloat NumRectangleEdges(const HPFloat row, const HPFloat column) const {
        return row * column;
    }
};
} // namespace kagen
