...
private:
    ...
    void outputRadiusStats() const {
        ...
        for (const auto& s: radius_stats_) {
            ...
            fprintf(
                fout, "%u %llu %llu %llu %f %llu %llu %f %llu %llu %i %llu %llu %llu\n", 3, config_.n, total_chunks_,
                chunks_per_dim_, chunk_size_, cells_per_chunk_, cells_per_dim_, cell_size_, max_radius_, s.first,
                s.second.radius, s.second.n_p_cell, s.second.n_p_tria, s.second.n_simplices);
        }
    ...
    void GenerateEdges(const SInt chunk_row, const SInt chunk_column, const SInt chunk_depth) override {
        ...
        Box_3d bbChunk(
            chunk_row * chunk_size_, chunk_column * chunk_size_, chunk_depth * chunk_size_,
            (chunk_row + 1) * chunk_size_, (chunk_column + 1) * chunk_size_, (chunk_depth + 1) * chunk_size_);
        ...
        for (SInt cell_row = 0; cell_row < cells_per_dim_; ++cell_row) {
            for (SInt cell_column = 0; cell_column < cells_per_dim_; ++cell_column) {
                for (SInt cell_depth = 0; cell_depth < cells_per_dim_; ++cell_depth) {
                    SInt cell_id =
                        cell_row * cells_per_dim_ + cell_column + (cells_per_dim_ * cells_per_dim_) * cell_depth;
        ...
        for (; !conflictFree && cell_radius <= max_radius_; ++cell_radius) {
            ...
            Box_3d bbNH(
                chunk_row * chunk_size_ - cell_radius * cell_size_,
            ...
            for (SSInt chunk_row_diff = -chunk_radius; chunk_row_diff <= (SSInt)chunk_radius; ++chunk_row_diff) {
                for (SSInt chunk_col_diff = -chunk_radius; chunk_col_diff <= (SSInt)chunk_radius; ++chunk_col_diff) {
                    for (SSInt chunk_dep_diff = -chunk_radius; chunk_dep_diff <= (SSInt)chunk_radius;
                         ++chunk_dep_diff) {
                        if ((SInt)std::max(
                                {std::abs(chunk_row_diff), std::abs(chunk_col_diff), std::abs(chunk_dep_diff)})
                            < chunk_radius)
                        ...
                        if (neighbor_chunk_row < 0) {
                            x_offset =
                                -1.0
                                * (std::ceil(std::abs(neighbor_chunk_row) / static_cast<LPFloat>(chunks_per_dim_)));
                        } else {
                            x_offset =
                                1.0
                                * (std::floor(std::abs(neighbor_chunk_row) / static_cast<LPFloat>(chunks_per_dim_)));
                        }
                        ...
                        if (neighbor_chunk_col < 0) {
                            y_offset =
                                -1.0
                                * (std::ceil(std::abs(neighbor_chunk_col) / static_cast<LPFloat>(chunks_per_dim_)));
                        } else {
                            y_offset =
                                1.0
                                * (std::floor(std::abs(neighbor_chunk_col) / static_cast<LPFloat>(chunks_per_dim_)));
                        }
                        ...
                        if (neighbor_chunk_dep < 0) {
                            z_offset =
                                -1.0
                                * (std::ceil(std::abs(neighbor_chunk_dep) / static_cast<LPFloat>(chunks_per_dim_)));
                        } else {
                            z_offset =
                                1.0
                                * (std::floor(std::abs(neighbor_chunk_dep) / static_cast<LPFloat>(chunks_per_dim_)));
                        }
            ...
            for (auto f = tria.finite_cells_begin(); f != tria.finite_cells_end(); ++f) {
                ...
                if (touchesChunk) {
                    Circ_3d c(
                        f->vertex(0)->point(), f->vertex(1)->point(), f->vertex(2)->point(), f->vertex(3)->point());
