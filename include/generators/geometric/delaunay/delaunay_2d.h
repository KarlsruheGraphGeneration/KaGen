...
public:
    CGALCairoPainter(const Point_2d& a, const Point_2d& b) : CairoPainter(a[0], a[1], b[0], b[1]) {}
    CGALCairoPainter(float x1 = 0, float y1 = 0, float x2 = 1, float y2 = 1) : CairoPainter(x1, y1, x2, y2) {}
    ...
    void drawCircle(const Circ_2d& c, bool dashed = false) {
        CairoPainter::drawCircle(c.center()[0], c.center()[1], std::sqrt(c.squared_radius()), dashed);
    }
};
#endif
...
public:
    Delaunay2D(PGeneratorConfig& config, const PEID rank, const EdgeCallback& cb)
