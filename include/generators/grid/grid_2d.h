...
class Grid2D {
public:
    Grid2D(PGeneratorConfig& config, const PEID rank, const PEID size)
        : config_(config),
          rng_(config),
          io_(config),
          rank_(rank),
...
private:
    ...
    bool IsLocalVertex(const SInt local_row, const SInt local_col, const SInt rows, const SInt cols) {
        if (local_row >= rows)
            return false;
        if (local_col >= cols)
            return false;
