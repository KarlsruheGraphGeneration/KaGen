...
private:
    ...
    void GenerateVertices(const SInt annulus_id, SInt chunk_id, const SInt cell_id) {
        bool clique = false;
    ...
    void QueryBoth(const SInt annulus_id, const SInt chunk_id, const SInt cell_id, const Vertex& q) {
    ...
    void
    Query(const SInt annulus_id, const SInt chunk_id, const SInt cell_id, const Vertex& q, bool search_down = true) {
        // Boundaries
        auto& annulus        = annuli_[ComputeGlobalChunkId(annulus_id, chunk_id)];
        ...
        if (std::get<1>(annulus) >= clique_thres_ && std::max(TotalGridSizeForAnnulus(annulus_id), config_.k) > 1) {
            ...
            if (current_max_phi_ > max_cell_phi
                ...
                GenerateVertices(annulus_id, next_chunk_id, next_cell_id);
                QueryLeftNeighbor(
                    annulus_id, next_chunk_id, next_cell_id, q, std::abs(max_cell_phi - 2 * M_PI) < cell_eps_,
                    search_down);
            }
        }
    ...
    void GenerateGridEdges(const SInt annulus_id, const SInt chunk_id, const SInt cell_id, const Vertex& q) {
    ...
    GetBoundaryPhis(const LPFloat boundary_phi, const LPFloat boundary_r, const SInt annulus_id) const {
    ...
    inline SInt ComputeGlobalCellId(const SInt annulus, const SInt chunk, const SInt cell) {
